...
// Delete pieces
if (action1Pressed && !oPuzzleBoard.inSpellBook){
	cast = false;
	
	// Set spell book to look at
	var ind = thisPlayerSpellBookIndex;
	var dat = global.spellData;
	
	// Check Spells
	switch(SpellChar("spellID", dat, ind)){
		// Single
		case 0: 
			coords = [
				[cursorX, cursorY]
		...
		case 6:
			...
			var pass = false;
			var c = coords[0, 0];
			if (ds_grid_get(g, c[0], c[1]) == 1 || ds_grid_get(g, c[0], c[1]) > 4) { // ignore items
				c = coords[0, 1];
				if (IsItem(c[0], c[1])) { // allow items
					for (var k = 0; k < array_length_1d(items); k++) // add item to list
						if (items[k] == 0)	{
				...
				if (ds_grid_get(g, c[0], c[1]) == 3 || ds_grid_get(g, c[0], c[1]) > 4) { // ignore items
					c = coords[0, 2];
					if (IsItem(c[0], c[1])) { // allow items
						for (var k = 0; k < array_length_1d(items); k++) // add item to list
							if (items[k] == 0)	{
